...
5. [How do I use bootstrap.dat with viad?](#Importing)
...
### 5. How do I use bootstrap.dat with viad?

viad comes with a separate utility named `addblock` which can be used to import
...
1. Stop viad if it is already running.  This is required since addblock needs to
   access the database used by viad and it will be locked if viad is using it.
