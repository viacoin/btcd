...
viad provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, viad supports a hybrid,
...
Configuring viad as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the viad command
line or in the viad configuration file.  Typically the Tor proxy address will be
...
By default, viad assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since viad will act as the server.
...
```text
HiddenServiceDir /var/tor/viad
HiddenServicePort 8333 127.0.0.1:8333
```

Once Tor is configured to provide the hidden service and you have obtained your
generated .onion address, configuring viad as a Tor hidden service requires
three flags:
...
```bash
$ ./viad --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
viad provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the viad command line or in the viad
...
```bash
$ ./viad --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
viad provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./viad --proxy=127.0.0.1:9050 --torisolation
```
