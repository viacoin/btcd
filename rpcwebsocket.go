...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *viautil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *viautil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *viautil.Tx, isNew bool) {
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) addAddress(a viautil.Address) {
	switch a := a.(type) {
...
func (f *wsClientFilter) addAddressStr(s string, params *chaincfg.Params) {
	...
	// script.
	a, err := viautil.DecodeAddress(s, params)
	if err != nil {
		return
	}
	f.addAddress(a)
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) existsAddress(a viautil.Address) bool {
	switch a := a.(type) {
	case *viautil.AddressPubKeyHash:
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) removeAddress(a viautil.Address) {
	switch a := a.(type) {
	case *viautil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected viautil.Block
type notificationBlockDisconnected viautil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *viautil.Tx,
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *viautil.Block) {
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *viautil.Tx) {
	...
	ntfn := btcjson.NewTxAcceptedNtfn(txHashStr, viautil.Amount(amount).ToBTC())
	marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
	if err != nil {
...
func blockDetails(block *viautil.Block, txIndex int) *btcjson.BlockDetails {
	if block == nil {
		return nil
	}
...
func newRedeemingTxNotification(txHex string, index int, block *viautil.Block) ([]byte, error) {
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	addrs map[string]map[chan struct{}]*wsClient, tx *viautil.Tx, block *viautil.Block) {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *viautil.Tx,
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	tx *viautil.Tx, block *viautil.Block) {
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		_, err := viautil.DecodeAddress(addr, params)
		if err != nil {
...
func rescanBlock(wsc *wsClient, lookups *rescanKeys, blk *viautil.Block) {
...
//
// NOTE: This extension is ported from github.com/decred/dcrd
func rescanBlockFilter(filter *wsClientFilter, block *viautil.Block, params *chaincfg.Params) []string {
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *viautil.Block) error {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	...
	for _, addrStr := range cmd.Addresses {
		addr, err := viautil.DecodeAddress(addrStr, params)
		if err != nil {
			jsonErr := btcjson.RPCError{
				Code: btcjson.ErrRPCInvalidAddressOrKey,
...
fetchRange:
	...
	// Notify websocket client of the finished rescan.  Due to how viad
