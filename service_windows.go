...
const (
	// svcName is the name of viad service.
	svcName = "viadsvc"
...
// logServiceStartOfDay logs information about viad when the main server has
...
// viadService houses the main service handler which handles all service
// updates and launching viadMain.
type viadService struct{}
...
// long-running viadMain (which is the real meat of viad), handles service
// change requests, and notifies the service control manager of changes.
func (s *viadService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start viadMain in a separate goroutine so the service can start
...
// installService attempts to install the viad service.  Typically this should
...
// removeService attempts to uninstall the viad service.  Typically this should
...
// startService attempts to start the viad service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &viadService{})
	if err != nil {
