...
type memWallet struct {
	...
	addrs map[uint32]viautil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]viautil.Address)
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*viautil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          viautil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (viautil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []viautil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt viautil.Amount, feeRate viautil.Amount) error {
	const (
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := viautil.Amount(txSize * int(feeRate))
...
//
// This function is safe for concurrent access.
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut, feeRate viautil.Amount) (*wire.MsgTx, error) {
	m.Lock()
	defer m.Unlock()
	...
	var outputAmt viautil.Amount
	for _, output := range outputs {
		outputAmt += viautil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() viautil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance viautil.Amount
...
func keyToAddr(key *btcec.PrivateKey, net *chaincfg.Params) (viautil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := viautil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
