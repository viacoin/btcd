...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt viautil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(viautil.Amount(5 * viautil.SatoshiPerBitcoin))
	...
	txid = genSpend(viautil.Amount(500 * viautil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(viautil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*viautil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, viautil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := viautil.Amount(250 * viautil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = viautil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := viautil.Amount(50 * viautil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := viautil.Amount(numMatureOutputs * 50 * viautil.SatoshiPerBitcoin)
