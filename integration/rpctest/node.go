...
// nodeConfig contains all the args, and data required to launch a viad process
...
// arguments returns an array of arguments that be used to launch the viad
// process.
...
// command returns the exec.Cmd which will be used to start the viad process.
...
// to the viad process that is launched via Start().
...
// as the base for the log and data directories for viad.
...
// start creates a new viad process, and writes its pid in a file reserved for
...
// stop interrupts the running viad process process, and waits until it exits
...
// shutdown terminates the running viad process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := viautil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}
