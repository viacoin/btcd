...
// Harness fully encapsulates an active viad process to provide a unified
// platform for creating rpc driven integration tests involving viad. The
// active viad node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active viad process is fully
...
func New(activeNet *chaincfg.Params, handlers *rpcclient.NotificationHandlers,
	...
	if handlers.OnFilteredBlockConnected != nil {
		...
		handlers.OnFilteredBlockConnected = func(height int32, header *wire.BlockHeader, filteredTxns []*viautil.Tx) {
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the viad node itself. This spawns a new process which will be
	// managed
	...
	// wallet.
	filterAddrs := []viautil.Address{h.wallet.coinbaseAddr}
	if err := h.Node.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure viad properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created viad
...
func (h *Harness) NewAddress() (viautil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() viautil.Amount {
...
func (h *Harness) SendOutputs(targetOutputs []*wire.TxOut,
	feeRate viautil.Amount) (*chainhash.Hash, error) {
...
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut,
	feeRate viautil.Amount) (*wire.MsgTx, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) GenerateAndSubmitBlock(txns []*viautil.Tx, blockVersion int32,
	blockTime time.Time) (*viautil.Block, error) {
...
func (h *Harness) GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
	txns []*viautil.Tx, blockVersion int32, blockTime time.Time,
	mineTo []wire.TxOut) (*viautil.Block, error) {
	...
	prevBlock := viautil.NewBlock(mBlock)
