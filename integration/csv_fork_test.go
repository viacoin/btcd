...
import (
	...
	"github.com/viacoin/viad/integration/rpctest"
...
func makeTestOutput(r *rpctest.Harness, t *testing.T,
	amt viautil.Amount) (*btcec.PrivateKey, *wire.OutPoint, []byte, error) {
	...
	// spend.
	a, err := viautil.NewAddressPubKey(key.PubKey().SerializeCompressed(), r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0113Activation(t *testing.T) {
	...
	viadCfg := []string{"--rejectnonstd"}
	r, err := rpctest.New(&chaincfg.SimNetParams, nil, viadCfg)
	if err != nil {
	...
	const outputValue = viautil.SatoshiPerBitcoin
	...
	txns := []*viautil.Tx{viautil.NewTx(tx)}
...
func createCSVOutput(r *rpctest.Harness, t *testing.T,
	numSatoshis viautil.Amount, timeLock int32,
	...
	p2shAddr, err := viautil.NewAddressScriptHash(csvScript, r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0068AndBIP0112Activation(t *testing.T) {
	...
	const (
		outputAmt         = viautil.SatoshiPerBitcoin
	...
	for txVersion := int32(0); txVersion < 3; txVersion++ {
		...
		txns := []*viautil.Tx{viautil.NewTx(spendingTx)}
		block, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
		if err != nil {
			t.Fatalf("unable to submit block: %v", err)
		}
	...
	for i, test := range tests {
		...
		if !test.accept {
			txns := []*viautil.Tx{viautil.NewTx(test.tx)}
			_, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
			if err == nil {
