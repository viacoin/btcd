...
func (sp *serverPeer) OnTx(_ *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a viautil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := viautil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a viautil.Block which provides some
	// convenience methods and things such as hash caching.
	block := viautil.NewBlockFromBlockAndBytes(msg, buf)
...
func (sp *serverPeer) OnFeeFilter(_ *peer.Peer, msg *wire.MsgFeeFilter) {
	...
	if msg.MinFee < 0 || msg.MinFee > viautil.MaxSatoshi {
		peerLog.Debugf("Peer %v sent an invalid feefilter '%v' -- "+
			"disconnecting", sp, viautil.Amount(msg.MinFee))
		sp.Disconnect()
		return
	}
...
func (s *server) TransactionConfirmed(tx *viautil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices,
			viadLookup, func(addrs []*wire.NetAddress) {
...
// newServer returns a new viad server configured to listen on addr for the
...
func newServer(listenAddrs []string, db database.DB, chainParams *chaincfg.Params) (*server, error) {
	...
	amgr := addrmgr.New(cfg.DataDir, viadLookup)
	...
	txC := mempool.Config{
		...
		CalcSequenceLock: func(tx *viautil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
