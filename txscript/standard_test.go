...
// newAddressPubKey returns a new viautil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) viautil.Address {
	addr, err := viautil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new viautil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) viautil.Address {
	addr, err := viautil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new viautil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) viautil.Address {
	addr, err := viautil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the viautil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the viautil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := viautil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := viautil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := viautil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := viautil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := viautil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*viautil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*viautil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*viautil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*viautil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*viautil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
