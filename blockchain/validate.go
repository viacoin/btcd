...
const (
	...
	baseSubsidy = 50 * viautil.SatoshiPerBitcoin
)
...
func IsCoinBase(tx *viautil.Tx) bool {
...
func IsFinalizedTransaction(tx *viautil.Tx, blockHeight int32, blockTime time.Time) bool {
	msgTx := tx.MsgTx()
...
func CheckTransactionSanity(tx *viautil.Tx) error {
	...
	for _, txOut := range msgTx.TxOut {
		...
		if totalSatoshi > viautil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
func checkProofOfWork(header *wire.BlockHeader, powLimit *big.Int, flags BehaviorFlags) error {
	...
	if flags&BFNoPoWCheck != BFNoPoWCheck {
		...
		hashNum := HashToBig(hash) //PowHash() returns &hash
...
// txscript.
func CountSigOps(tx *viautil.Tx) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *viautil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func checkBlockSanity(block *viautil.Block, powLimit *big.Int, timeSource MedianTimeSource, flags BehaviorFlags) error {
...
// CheckBlockSanity performs some preliminary checks on a block to ensure it is
// sane before continuing with block processing.  These checks are context free.
func CheckBlockSanity(block *viautil.Block, powLimit *big.Int, timeSource MedianTimeSource) error {
...
func ExtractCoinbaseHeight(coinbaseTx *viautil.Tx) (int32, error) {
...
func checkSerializedHeight(coinbaseTx *viautil.Tx, wantHeight int32) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkBlockContext(block *viautil.Block, prevNode *blockNode, flags BehaviorFlags) error {
	// The genesis block is valid by definition.
	if prevNode == nil {
		return nil
	}
...
func (b *BlockChain) checkBIP0030(node *blockNode, block *viautil.Block, view *UtxoViewpoint) error {
...
func CheckTransactionInputs(tx *viautil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		if originTxSatoshi < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", viautil.Amount(originTxSatoshi))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxSatoshi > viautil.MaxSatoshi {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				viautil.Amount(originTxSatoshi),
				viautil.MaxSatoshi)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if totalSatoshiIn < lastSatoshiIn ||
			totalSatoshiIn > viautil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block *viautil.Block, view *UtxoViewpoint, stxos *[]spentTxOut) error {
...
func (b *BlockChain) CheckConnectBlock(block *viautil.Block) error {
	b.chainLock.Lock()
