...
// exceeded.
func (b *BlockChain) addOrphanBlock(block *viautil.Block) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) CalcSequenceLock(tx *viautil.Tx, utxoView *UtxoViewpoint, mempool bool) (*SequenceLock, error) {
	b.chainLock.Lock()
...
func (b *BlockChain) calcSequenceLock(node *blockNode, tx *viautil.Tx, utxoView *UtxoViewpoint, mempool bool) (*SequenceLock, error) {
...
// already there.
func dbMaybeStoreBlock(dbTx database.Tx, block *viautil.Block) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block *viautil.Block, view *UtxoViewpoint, stxos []spentTxOut) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) disconnectBlock(node *blockNode, block *viautil.Block, view *UtxoViewpoint) error {
...
func countSpentOutputs(block *viautil.Block) int {
...
func (b *BlockChain) reorganizeChain(detachNodes, attachNodes *list.List, flags BehaviorFlags) error {
	...
	detachBlocks := make([]*viautil.Block, 0, detachNodes.Len())
	detachSpentTxOuts := make([][]spentTxOut, 0, detachNodes.Len())
	attachBlocks := make([]*viautil.Block, 0, attachNodes.Len())
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBestChain(node *blockNode, block *viautil.Block, flags BehaviorFlags) (bool, error) {
...
type IndexManager interface {
	...
	// main chain.
	ConnectBlock(database.Tx, *viautil.Block, *UtxoViewpoint) error

	// DisconnectBlock is invoked when a block has been disconnected from
	// the main chain.
	DisconnectBlock(database.Tx, *viautil.Block, *UtxoViewpoint) error
}
