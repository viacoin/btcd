...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: viautil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := viautil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*viautil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, viautil.NewTx(tx))
	}
...
func additionalCoinbase(amount viautil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee viautil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee viautil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee viautil.Amount) *wire.MsgTx {
