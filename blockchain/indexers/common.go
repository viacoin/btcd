...
type Indexer interface {
	...
	ConnectBlock(dbTx database.Tx, block *viautil.Block, view *blockchain.UtxoViewpoint) error

	// DisconnectBlock is invoked when the index manager is notified that a
	// block has been disconnected from the main chain.
	DisconnectBlock(dbTx database.Tx, block *viautil.Block, view *blockchain.UtxoViewpoint) error
}
