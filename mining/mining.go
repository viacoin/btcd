...
const (
	...
	MinHighPriority = viautil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr viautil.Address) (*viautil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *viautil.Tx, height int32) error {
...
func logSkippedDeps(tx *viautil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress viautil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*viautil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := viautil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := viautil.NewBlock(&msgBlock)
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A viautil.Block should use saved in the state to avoid
