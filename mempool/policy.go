...
const (
	...
	DefaultMinRelayTxFee = viautil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee viautil.Amount) int64 {
	...
	if minFee < 0 || minFee > viautil.MaxSatoshi {
		minFee = viautil.MaxSatoshi
	}
...
func checkInputsStandard(tx *viautil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee viautil.Amount) bool {
...
func checkTransactionStandard(tx *viautil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee viautil.Amount,
...
func GetTxVirtualSize(tx *viautil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
